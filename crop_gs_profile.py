...
new_dir = Path('D:/rectify/manual_profile')
...
def main(args):
    ...
    tree = [(*t[:2], *list(t[2:5] - soma + shift), *t[5:]) for t in tree]
    tree = swc_handler.trim_swc(tree, crop_box)
    swc_handler.write_swc(tree, out_path)


if __name__ == '__main__':
    gs_dir = Path(r'Z:\SEU-ALLEN\Projects\fullNeurons\V2023_01_10\profiled')
    new_dir.mkdir(exist_ok=True)
    arglist = [*gs_dir.glob("*.eswc")]
    with Pool(12) as p:
