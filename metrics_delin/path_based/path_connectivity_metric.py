#================================================================
#   Filename     : path_connectivity_metric.py
#
#================================================================
import numpy as np
...
def closest_points_on_edges(edges, points):
    s = edges[:, np.newaxis, 0:3]  ## 0:3
    t = edges[:, np.newaxis, 3:6]  ## 3:6

    ## assert edges.shape[-1] % 2 != 0, "shape of pair's data not equals"
    ## s = edges[:, np.newaxis, 0:int(edges.shape[-1]//2)]
    ## t = edges[:, np.newaxis, int(edges.shape[-1]//2):int(edges.shape[-1])]
    # print(points)
...
def get_candidate_path_node_positions_5_nogpu(g, path, thresholdExisting, thresholdSnap):
    ...
    for ei in range(len(edges)):
        ...
        while i < len(ie) and ae[ie[i]] <= 1 and \
                np.sum(np.array(ce[ie[i]]) - np.array(last_pos)) ** 2 > thExst2:
            # (ce[ie[i]][0] - last_pos[0]) ** 2 + (ce[ie[i]][1] - last_pos[1]) ** 2 > thExst2:  ## np.sum((np.array(ce[ie[i]])-np.array(last_pos))**2 > thExst2:
            # while far enough from edge end point
            ii = ie[i]
            pos = ce[ii]
            # d2 = (pos[0] - prev_pos[0]) ** 2 + (pos[1] - prev_pos[1]) ** 2  ## np.sum((np.array(pos)-np.array(prev_pos))**2)
            d2 = np.sum((np.array(pos) - np.array(prev_pos)) ** 2)
            if d2 > thExst2:
                largest_node += 1
                g.add_node(largest_node, pos=(pos[0], pos[1], pos[2]))
...
def sample_chain(g):
    ...
    while len(chain) > 0:
        ...
        if len(g[n]) == 1 and g.edges[(n, list(g.neighbors(n))[0])]['state'] == EdgeState.onPath:
            ...
            neighborsOnUnseenEdges = [m for m in g.neighbors(n) if
                                      g.edges[(m, n)]['state'] == EdgeState.unseen and n != m]
...
def calcPathLength(p):
    ...
    prev = next(ni)
    for curr in ni:
        # l += ((curr[0] - prev[0]) ** 2 + (curr[1] - prev[1]) ** 2) ** 0.5  ## l += np.linalg.norm(np.array(curr)-np.array(prev), ord=2)
        l += np.linalg.norm(np.array(curr) - np.array(prev), ord=2)
        prev = curr
    return l
...
def new2l2s_v0(g, h, th_existing=1, th_snap=25):
    ...
    for p in paths:
        ...
        # get pairwise costs
        pairwiseCosts, allowedConnections, shortestPaths, sptime, sptim2 = get_pairwise_costs_3(g, newpath,
                                                                                                cand_positions,
