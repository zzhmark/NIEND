# ================================================================
#   Filename     : junction_new_metric.py
#
# ================================================================
import numpy as np
...
def matching_with_snapping(junctions_g, junctions_pos_g, G, H, th_existing=1, th_snap=25, alpha=100):
    # snap the node of graph G into graph H if they are sufficiently close 将图G的节点折入图H，如果它们足够接近
    H_snap, corresps = utils.snap_points_to_graph(H, junctions_pos_g, th_existing=th_existing, th_snap=th_snap)

    # get control nodes 控制节点
    ...
    # find some candidates control points to match 找一些候选控制点来匹配
...
def opt_j(G_gt, G_pred, th_existing=1, th_snap=25, alpha=100):
    ...
    if utils.is_empty(G_gt):  # 如果gt空 报警
        raise ValueError("Ground-truth graph is empty!")

    if utils.is_empty(G_pred):  # 如果预测空，所有参数为0，图像默认 返回
