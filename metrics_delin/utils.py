...
__all__ = ["json_read", "json_write", "pickle_read", "pickle_write",
           ...
           "plot_graph", "load_graph_txt", "load_graph_swc", "save_graph_txt", "oversampling_graph",
...
def load_graph_swc(filename):
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if(lsp[0][0] == '#'):continue
            # print(lsp)
            x, y, z = lsp[2], lsp[3], lsp[4]
            # print(G.nodes)
            # time.sleep(5)
            # print(int(lsp[0]), float(x), float(y), float(z))
            G.add_node(int(lsp[0]), pos=(float(x), float(y), float(z)))
            if(int(lsp[6]) == -1 or int(lsp[6]) == 0 or int(lsp[6]) == int(lsp[0])):continue
            G.add_edge(int(lsp[0]), int(lsp[6]))
...
def interpolate_new_nodes(p1, p2, spacing=2):
    _p1 = np.reshape(p1, (len(p1),))
    _p2 = np.reshape(p2, (len(p2),))
...
def undersampling_graph(G, spacing=10, inplace=False):
    ...
    def distance(g, n1, n2):
        ...
        return np.linalg.norm(np.array(g.nodes[n1]['pos'])-np.array(g.nodes[n2]['pos']),ord=2)
...
def shift_graph(G, shift_x, shift_y, shift_z):
    G_ = G.copy()
    for _, data in G_.nodes(data=True):
        x, y, z = data['pos']
        x, y, z = x + shift_x, y + shift_y, z + shift_z
        if isinstance(data['pos'], np.ndarray):
            data['pos'] = np.array([x, y, z])
        else:
            data['pos'] = (x, y, z)
    return G_


def crop_graph_naive(G, xmin=None, ymin=None, xmax=None, ymax=None, zmin=None, zmax=None):
    G_ = G.copy()
    for n in list(G_.nodes()):
        p = G_.nodes[n]['pos']
        if p[0] >= xmin and p[0] < xmax and p[1] >= ymin and p[1] < ymax and p[2] >= zmin and p[2] < zmax:
            pass
        else:
            G_.remove_node(n)
    return G_
...
def segments_intersection_point(line1, line2):     ## not stable
    xdiff = (line1[0][0] - line1[1][0], line2[0][0] - line2[1][0])
    ydiff = (line1[0][1] - line1[1][1], line2[0][1] - line2[1][1])
    # zdiff = (line1[0][2] - line1[1][2], line2[0][2] - line2[1][2])
    # print(line1, line2)

    def det(a, b):
        return a[0] * b[1] - a[1] * b[0]
    # print(xdiff,ydiff,zdiff)
    # div = np.linalg.det(xdiff, ydiff, zdiff)
    div = det(xdiff, ydiff)
    if div == 0:
        return None
    ...
    # z = det(d, zdiff) / div
    # plt.figure()
    # plt.plot(line1[0][:2],line1[1][0:2])
    # plt.plot(line2[0][:2], line2[1][0:2])
    # plt.scatter(x,y)
    # plt.show()
    # input(".......________>>>>>>")
    # print("line1", line1)
...
def segment_intersection_point_to_box(segment, xmin, ymin, zmin, xmax, ymax, zmax):
    bs = [((xmin, ymin, zmin), (xmin, ymax, zmin)),
    ...
    p1_out = p1[0] < xmin or p1[0] >= xmax or p1[1] < ymin or p1[1] >= ymax or p1[2] < zmin or p1[2] >= zmax
    p2_out = p2[0] < xmin or p2[0] >= xmax or p2[1] < ymin or p2[1] >= ymax or p2[2] < zmin or p2[2] >= zmax
...
def crop_graph(G, xmin=None, ymin=None, zmin=None, xmax=None, ymax=None, zmax=None):
    G_ = G.copy()
    for s, t in list(G_.edges()):
        p1 = G_.nodes[s]['pos']  # 相互连接的两个点的坐标
        p2 = G_.nodes[t]['pos']
        ...
        if p1_out and p2_out:
            ...
            new_seg = segment_intersection_point_to_box((p1, p2), xmin, ymin, zmin, xmax, ymax, zmax)
...
def uniform_node_sampling(G, dist_matching=25, max_node_probe=10000):
    ...
    xmin, ymin, zmin = nodes_pos.min(0)
    xmax, ymax, zmax = nodes_pos.max(0)
...
def uniform_node_sampling_with_snapping(G, dist_matching=25):
    ...
    xmin, ymin, zmin = nodes_pos_gt.min(0)
    xmax, ymax, zmax = nodes_pos_gt.max(0)
    ...
    for _ in range(100):
        ...
        zs = np.random.uniform(low=zmin, high=zmax, size=100)
        random_positions = np.vstack([xs, ys, zs]).T
...
def insert_nodes_in_edge(G, s, t, nodes, nodes_pos):
    ...
    def distance(idx):
        ...
        return np.linalg.norm(np.array(G_.nodes[s]['pos'])-np.array(nodes_pos[idx]), ord=2)
